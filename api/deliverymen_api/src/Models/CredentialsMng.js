...
async function create(params) {
    ...
    const isLoginUnic = await db.Credentials.findOne({ where: { CredentialLogin: params.CredentialLogin, CredentialUserRole: 3 } });
    if (isLoginUnic) throw new Error("login already taken !");

    const isMailUnic = await db.Credentials.findOne({ where: { CredentialEmail: params.CredentialEmail, CredentialUserRole: 3 } });
    if (isMailUnic) throw new Error("E-Mail already taken !");

    const isPhoneUnic = await db.Credentials.findOne({ where: { CredentialPhone: params.CredentialPhone, CredentialUserRole: 3 } });
